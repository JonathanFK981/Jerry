...
class STrack(BaseTrack):
    ...
    def __init__(self, tlwh, score, category_id):
...
class BYTETracker(object):
    ...
    def update(self, output_results, img_info, img_size):
        ...
        cid_second = cls[inds_second]           #   低分检测框的类别
        cid = cls[remain_inds]

        if len(dets) > 0:
            '''Detections'''
            detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, i) for
                          (tlbr, s, i) in zip(dets, scores_keep, cid)]
        else:
            detections = []
        ...
        if len(dets_second) > 0:
            ...
            detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, i) for
                          (tlbr, s, i) in zip(dets_second, scores_second, cid_second)]
        else:
            detections_second = []
